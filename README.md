...
## Loading a diagram from a JSON file

Instead of writing the IR inline, a fence can point at an external `.json` file with a `file="..."` attribute — handy for large diagrams, or ones generated by another tool:

````markdown
```archify file="./diagrams/web-app.architecture.json"
```
````

The path is resolved relative to the markdown/MDX file it appears in (not the project root or the process's working directory). It works together with an explicit type in the fence's language string too:

````markdown
```archify:sequence file="./diagrams/checkout.sequence.json"
```
````

An absolute path is also accepted and used as-is. A missing or unreadable file behaves like any other render failure: a visible inline error block by default, or a failed build with `strict: true`.
