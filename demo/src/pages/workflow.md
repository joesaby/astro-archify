...
Archify's own `agent-tool-call.workflow.json` example (reused here under Archify's MIT license). This uses **schema v2** and the constraint-driven workflow compiler shipped in Archify 2.16.0 — the canvas sizes itself; no fixed `viewBox` is required.
