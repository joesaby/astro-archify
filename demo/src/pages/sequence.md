...
Archify's own `cache-miss-request.sequence.json` example (reused here under Archify's MIT license). Note the explicit `archify:sequence` fence — the type override works even though the IR itself already declares `diagram_type: "sequence"`.
